...
- Windows Build Fix 14 targets the final two assertions from Windows run 32589047940 without weakening release gates: the whole-pixel Transform fixture now starts outside the real 7 px guide-drag hit zone so its half-pixel guide remains a snap target rather than stealing the gesture, and the RGBA8 Vector Feather/GPU colour carrier keeps exact zero-Feather RGB for ordinary one-code quantisation differences but falls back to canonical RGBA64-derived RGB at antialiased fill/stroke blend pixels where Qt's 8- and 16-bit painters materially disagree, preserving cross-depth visible parity without reducing 16-bit output precision.
- Windows Build Fix 15 addresses the sole remaining assertion from Windows run 32591062769 without weakening release gates: private session-cache image restore now explicitly zeroes the complete QImage allocation before copying persisted active pixels, because QImage::fill() does not guarantee row-padding bytes. This makes the existing "padding is never persisted" contract deterministic on Windows as well as Linux and prevents uninitialised process memory from surviving in restored image padding.
- Windows Build Fix 16 moves beyond the now-green 15/15 Windows CTest suite into portable deployment hardening: the required Windows platform and JPEG plugins are explicit additive Qt deployment roots, the staged executable receives a direct `windeployqt` runtime/plugin pass after CMake install, and portable verification now always emits structured preflight/launch diagnostics plus install, deployment, verifier and file-manifest logs when the executable cannot produce its own smoke report. No application rendering, persistence, test tolerance, or project-schema behaviour is changed.
