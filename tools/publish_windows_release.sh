#!/usr/bin/env bash
# Commit the current VFX Photo Lab release, build Windows packages on GitHub,
...
Before running, update:
  - VFXPHOTOLAB_RELEASE_VERSION in CMakeLists.txt
...
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null)" || fail "Run this inside the VFXPhotoLab Git repository."
cd "$REPO_ROOT"
...
# CI failure dumps and downloaded diagnostics are maintainer-local, never
# release source. Remove any copies that were accidentally tracked by an older
# run before the all-changes release commit below; ignored/untracked copies
# remain local.
...
echo
echo "Preparing VFX Photo Lab $VERSION ($TAG)"
echo "Branch: $BRANCH"
echo
...
gh run watch "$RUN_ID" --compact --exit-status || fail "Windows build failed. No public release was published."
...
echo
echo "Published VFX Photo Lab $VERSION successfully."
